...
class SettingsViewModel: ObservableObject {
  ...
  func prepareExport(allLifts: [LiftEvent]) {
    ...
    let exportableData = allLifts.map {
      LiftExportModel(
        date: $0.date,
  ...
  func handleImport(result: Result<[URL], Error>, modelContext: ModelContext) {
    do {
      ...
      if selectedFile.startAccessingSecurityScopedResource() {
        ...
        let importedLifts = try decoder.decode(
          [LiftExportModel].self,
          from: data
        )

        for lift in importedLifts {
          let newEvent = LiftEvent(
            date: lift.date,
        ...
        importAlertMessage =
          "Successfully imported \(importedLifts.count) lifts."
