...
struct PercentageReferenceView: View {
  ...
  let percentages = Array(stride(from: 105, through: 30, by: -5)).reversed()
    .reversed()  // 105 down to 30
  ...
  private func calculateWeight(percentage: Int) -> Double {
    let theoreticalMaxForReps =
      oneRepMax * (37.0 - Double(selectedRepBasis)) / 36.0
